...
impl CharBuffer {
    ...
    pub fn resize_and_clear(&mut self, size: impl Into<Offset>, cell: Option<Cell>) {
        let size: Offset = size.into();

        if size != self.size {
            self.buf.clear();
            self.buf.extend(std::iter::repeat(cell).take(size.area()));
            self.size = size;
        } else {
            self.buf.fill(cell);
        }
...
impl<Idx: Into<Offset>> Index<Idx> for CharBuffer {
...
impl<Idx: Into<Offset>> IndexMut<Idx> for CharBuffer {
